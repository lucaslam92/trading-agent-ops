...
OUT = ROOT / 'logs' / 'donchian_adx_compare_2023_2024.json'
...
results = []
for adx_window, adx_threshold in combos:
    for label, start, end in years:
        name = f'cmp_donchian_adx_{adx_window}_{adx_threshold}_{label}'
        cfg = {
            ...
            'strategy_setting': {
                ...
                'adx_window': adx_window, 'adx_threshold': adx_threshold, 'stop_loss_pct': 0.015,
                'next_bar_entry': True, 'base_slippage': 5.0, 'atr_slippage_multiplier': 0.05
            }
        }
        ...
        text = (proc.stdout or '') + '\n' + (proc.stderr or '')
        row = {'adx_window': adx_window, 'adx_threshold': adx_threshold, 'year': label, 'exit_code': proc.returncode}
