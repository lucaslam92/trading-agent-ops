...
CONFIG_DIR = ROOT / "configs" / "_optimize"
RESULT_PATH = ROOT / "logs" / "trend_optimization_results.json"
...
for fast, slow, atr, stop in itertools.product(fast_windows, slow_windows, atr_multipliers, stop_losses):
    ...
    cfg_path = CONFIG_DIR / f"trend_{combo_id:03d}.json"
    ...
    results.append(item)
    print(f"[{combo_id:03d}] fast={fast} slow={slow} atr={atr} stop={stop} -> return={item.get('total_return')} sharpe={item.get('sharpe_ratio')}")

# sort: higher total return, then higher sharpe, then smaller drawdown magnitude
