...
RESULT_PATH = ROOT / "logs" / "donchian_optimization_results.json"

breakout_windows = [20, 30, 50]
...
results = []
combo_id = 0
for breakout, exit_w, atr, stop in itertools.product(breakout_windows, exit_windows, atr_multipliers, stop_losses):
    if exit_w >= breakout:
        continue
    combo_id += 1
    name = f"opt_donchian_{combo_id:03d}"
    cfg = {
        "mode": "backtest",
    ...
    results.append(item)
    print(f"[{combo_id:03d}] breakout={breakout} exit={exit_w} atr={atr} stop={stop} -> return={item.get('total_return')} sharpe={item.get('sharpe_ratio')}")
