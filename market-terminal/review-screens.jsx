/* ─────────────────────────────────────────────────────────────────────────
   ...
   Requires: store.js (window.MarketStore), components.jsx, data.js
   Exports: ChecklistScreen, JournalScreen, HeaderCountdown
   ───────────────────────────────────────────────────────────────────────── */
const { useState: _rS, useEffect: _rE, useMemo: _rM, useRef: _rR } = React;
const _store = () => window.MarketStore;
...
function JournalScreen() {
  const s = useMarket();
  const [form, setForm] = _rS(() => EMPTY(_store().today()));
  const [history, setHistory] = _rS([]);
  const [flash, setFlash] = _rS('');

  const set = (k, v) => setForm((f) => ({ ...f, [k]: v }));
  const loadHistory = async () => { const all = await _store().getAll('journals'); all.sort((a, b) => b.date.localeCompare(a.date)); setHistory(all.slice(0, 30)); };

  _rE(() => { _store().get('journals', form.date).then((d) => { if (d) setForm({ ...EMPTY(form.date), ...d }); }); loadHistory(); }, []);

  const onDate = async (date) => { const d = await _store().get('journals', date); setForm(d ? { ...EMPTY(date), ...d } : EMPTY(date)); };
  const saveJournal = async () => { await _store().save('journals', form); setFlash('已保存'); setTimeout(() => setFlash(''), 1800); loadHistory(); };
  const newToday = () => setForm(EMPTY(_store().today()));

  // pull live market read into the form
  const pullLive = () => {
    const themes = window.MarketData.themeAgg();
    ...
    setForm((f) => ({
      ...
      analysis: f.analysis || `资金主线流入 ${themes[0].theme}（${fmtFlow(themes[0].flow)}亿），流出 ${themes[themes.length - 1].theme}。`,
    }));
