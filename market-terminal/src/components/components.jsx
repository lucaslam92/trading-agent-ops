/* ─────────────────────────────────────────────────────────────────────────
   components.jsx — shared terminal UI atoms + viz
   ───────────────────────────────────────────────────────────────────────── */
import { useEffect, useMemo, useState } from 'react';
import { MarketData } from '../lib/data.js';
import { cls, cssVar, fmtFlow, fmtPct } from '../lib/market-utils.js';

/* live subscription hook — re-renders on every tick */
export function useMarket() {
  const [, force] = useState(0);
  useEffect(() => MarketData.subscribe(() => force((n) => n + 1)), []);
  return MarketData.state;
}

/* ── Pct: colored percentage ──────────────────────────────────────────────── */
export function Pct({ v, size = 13, arrow = false, weight = 600 }) {
  return (
...
/* ── Sparkline ────────────────────────────────────────────────────────────── */
export function Spark({ data, w = 64, h = 22, color, strokeW = 1.5, fill = false }) {
...
export function FlowBar({ v, max, h = 8 }) {
...
/* ── Index ticker strip ───────────────────────────────────────────────────── */
export function IndexStrip({ compact = false }) {
  const s = useMarket();
  return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', height: 10, borderRadius: 5, overflow: 'hidden', background: 'var(--panel-2)' }}>
        <div style={{ width: (up / total) * 100 + '%', background: 'var(--up)', transition: 'width .7s' }} />
        <div style={{ width: (flat / total) * 100 + '%', background: 'var(--flat)', opacity: 0.5 }} />
        <div style={{ width: (down / total) * 100 + '%', background: 'var(--down)', transition: 'width .7s' }} />
      </div>
...
export function RotationMap({ width = 460, height = 360, labeled = true }) {
  const s = useMarket();
  ...
  return (
    <svg width={width} height={height} style={{ display: 'block' }}>
      ...
      {s.sectors.map((sec, i) => {
        ...
        return (
          <g key={sec.id || `${sec.name}-${i}`} style={{ transition: 'transform .9s cubic-bezier(.4,0,.2,1)', transform: `translate(${cx}px,${cy}px)` }}>
...
export function SectorFlowRanking({ limit = 12, showSpark = false }) {
  const s = useMarket();
  ...
  return (
    <div>
      {top.map((sec, i) => (
        <div key={sec.id || `${sec.name}-${i}`} style={{ display: 'grid', gridTemplateColumns: showSpark ? '78px 1fr 56px 60px' : '88px 1fr 64px', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < top.length - 1 ? '1px solid var(--line-soft)' : 'none' }}>
...
export function ThemeTape() {
  const s = useMarket();
  const themes = useMemo(() => MarketData.themeAgg(), [s.updatedAt]);
  if (!themes.length) {
    return <div style={{ fontSize: 12, color: 'var(--ink-4)', padding: '4px 0' }}>等待行情数据...</div>;
  }
...
/* ── Leader board ─────────────────────────────────────────────────────────── */
export function LeaderBoard({ dense = false, spark = true }) {
  const s = useMarket();
