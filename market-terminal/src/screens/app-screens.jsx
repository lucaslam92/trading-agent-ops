/* ─────────────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────────────── */
import { useEffect as _aE, useMemo as _aM, useRef as _aR, useState as _aS } from 'react';
import { MarketData } from '../lib/data.js';
import { cssVar, fmtPct } from '../lib/market-utils.js';
import {
  BreadthBar,
  GlobalStrip,
  IndexStrip,
...
export function ToolboxScreen() {
  const s = useMarket();
  const [sel, setSel] = _aS('');
  _aE(() => {
    if (!sel && s.leaders[0]?.code) setSel(s.leaders[0].code);
  }, [s.leaders, sel]);
  if (!s.leaders.length) {
    return (
      <div className="panel" style={{ padding: 24, fontSize: 13, color: 'var(--ink-3)' }}>
        等待行情数据...
      </div>
    );
  }
